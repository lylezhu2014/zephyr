...
STRUCT_PACKED_PRE struct bt_monitor_hdr {
	uint16_t  data_len;
	uint16_t  opcode;
	uint8_t   flags;
	uint8_t   hdr_len;
...
STRUCT_PACKED_PRE struct bt_monitor_ts32 {
	uint8_t   type;
	uint32_t  ts32;
} STRUCT_PACKED_POST;

STRUCT_PACKED_PRE struct bt_monitor_new_index {
	uint8_t  type;
	uint8_t  bus;
	uint8_t  bdaddr[6];
	char  name[8];
...
STRUCT_PACKED_PRE struct bt_monitor_user_logging {
	uint8_t  priority;
	uint8_t  ident_len;
} STRUCT_PACKED_POST;
