...
STRUCT_PACKED_PRE struct bt_sdp_data_elem_seq {
	uint8_t  type; /* Type: Will be data element sequence */
	uint16_t size; /* We only support 2 byte sizes for now */
} STRUCT_PACKED_POST;

STRUCT_PACKED_PRE struct bt_sdp_hdr {
	uint8_t  op_code;
	uint16_t tid;
	uint16_t param_len;
} STRUCT_PACKED_POST;

STRUCT_PACKED_PRE struct bt_sdp_svc_rsp {
	uint16_t  total_recs;
	uint16_t  current_recs;
} STRUCT_PACKED_POST;

STRUCT_PACKED_PRE struct bt_sdp_att_rsp {
...
STRUCT_PACKED_PRE struct bt_sdp_pdu_cstate {
	uint8_t length;
