...
STRUCT_PACKED_PRE struct bt_smp_hdr {
	uint8_t  code;
} STRUCT_PACKED_POST;
...
STRUCT_PACKED_PRE struct bt_smp_pairing {
...
STRUCT_PACKED_PRE struct bt_smp_pairing_confirm {
	uint8_t  val[16];
} STRUCT_PACKED_POST;

#define BT_SMP_CMD_PAIRING_RANDOM		0x04
STRUCT_PACKED_PRE struct bt_smp_pairing_random {
	uint8_t  val[16];
} STRUCT_PACKED_POST;

#define BT_SMP_CMD_PAIRING_FAIL			0x05
STRUCT_PACKED_PRE struct bt_smp_pairing_fail {
	uint8_t  reason;
} STRUCT_PACKED_POST;

#define BT_SMP_CMD_ENCRYPT_INFO			0x06
STRUCT_PACKED_PRE struct bt_smp_encrypt_info {
	uint8_t  ltk[16];
} STRUCT_PACKED_POST;

#define BT_SMP_CMD_MASTER_IDENT			0x07
STRUCT_PACKED_PRE struct bt_smp_master_ident {
	uint8_t ediv[2];
	uint8_t rand[8];
} STRUCT_PACKED_POST;

#define BT_SMP_CMD_IDENT_INFO			0x08
STRUCT_PACKED_PRE struct bt_smp_ident_info {
	uint8_t  irk[16];
} STRUCT_PACKED_POST;

#define BT_SMP_CMD_IDENT_ADDR_INFO		0x09
STRUCT_PACKED_PRE struct bt_smp_ident_addr_info {
	bt_addr_le_t addr;
} STRUCT_PACKED_POST;

#define BT_SMP_CMD_SIGNING_INFO			0x0a
STRUCT_PACKED_PRE struct bt_smp_signing_info {
	uint8_t csrk[16];
} STRUCT_PACKED_POST;

#define BT_SMP_CMD_SECURITY_REQUEST		0x0b
STRUCT_PACKED_PRE struct bt_smp_security_request {
	uint8_t  auth_req;
} STRUCT_PACKED_POST;

#define BT_SMP_CMD_PUBLIC_KEY			0x0c
STRUCT_PACKED_PRE struct bt_smp_public_key {
	uint8_t x[32];
	uint8_t y[32];
} STRUCT_PACKED_POST;

#define BT_SMP_DHKEY_CHECK			0x0d
STRUCT_PACKED_PRE struct bt_smp_dhkey_check {
	uint8_t e[16];
} STRUCT_PACKED_POST;
